...
chunk_output_type: console
bibliography: ../manuscript/assets/supplements.bib
csl: ../manuscript/assets/pnas.csl
---
...
library(clubSandwich) # for statistical test
library(patchwork)
library(scales)
library(scatterpie) # for scatterpie plot fig.4
library(tibble)
library(stringr)
library(deSolve)
library(latex2exp)
...
library(kableExtra)
library(ggbeeswarm)
library(faux) # for sampling, justify q2
# Set plot theme
...
# Appendix 3: `R` code to recreate all results 


***We recommend viewing this Appendix as an html file, available as a supporting file with this submission***  


## Define useful functions to run the analyses downstream

Please click on "code" to the right to see the functions we have written to help run the analyses. 
...
calculate_effect_sizes <- function(data){
  ...
  data_with_SDFD <- data %>% 
    mutate( # mean stabilization, which is also -1/2 of Is 
      ...
      IS = -2*mean_SD,
      # variance of SD effect size
      # it is a quarter of the var(rrIs) in Crawford et al. 2019
      ...
      n_min_SD = as.integer(pmin(`AinA N`, `AinB N`, `BinA N`, `BinB N`)),
      
      # next, fitness difference (FD) effect size
      mean_FD = 1/2*(log(`AinA mean`) + log(`AinB mean`) - 
                       2*log(`AinR mean`) - log(`BinA mean`) - log(`BinB mean`) 
                     + 2*log(`BinR mean`) ),
      ...
      sem_FD = sqrt(var_FD),
      n_min_FD = as.integer(pmin(n_min_SD, `AinR N`, `BinR N`)),
...
simulation_ci <- function(data_with_SDFD) {
  ...
  # draw from biomasses directly, to account for correlation
  # code from Gaurav
...
## Read in datasets
For this analysis, we are working with two complementary datasets. The first is the dataset of SD/FD effect sizes with field live soil as the reference; the second set of effect sizes are from studies with sterile soils as reference inoculua. 

```{r import-data, eval = T, include = F}
# Start with the FL ref
live_ref <- read_csv("data/combined_FL_unfiltered.csv")
# Let's clean up the dataframe to exclude unused columns
live_ref <- live_ref %>%
  select(-Origin,-`Mycorrhizal status`, -`Nitrogen fixer`, 
         -`Phylogenetic distance`, -`Precipitation`, -Ecosystem,
         -`Absolute latitude`, -`Family A`, -`Family B`, -rrIs, -`Variance(rrIs)`) %>%
  rename(species_pair = `Species pair`)

# Import the Sterile Soil ref
sterile_ref <- read_csv("data/combined_XS_unfiltered.csv")
# Let's clean up the dataframe to exclude unused columns
sterile_ref <- sterile_ref %>%
  # filter(rrIs > -4) %>%
  select(-Origin,-`Mycorrhizal status`, -`Nitrogen fixer`, 
         -`Phylogenetic distance`, -`Precipitation`, -Ecosystem,
         -`Absolute latitude`, -`Family A`, -`Family B`, -rrIs, -`Variance(rrIs)`)  %>%
...
# Fraction of inoculim (defining the cutoff as 25%)
...
We can first get a lay of the land on this dataset by checking how many species there are, from how many experiments, and from how many studies:
...
We can also do some summaries of the 4-5 moderators we are going to use later on in the analysis. First for the **Live reference** dataset: 
...
live_ref %>%
  mutate(inoc_frac_binary = ifelse(`Fraction inoculum` < 0.251, "low", "high")) %>%
...
Before starting on the meta-analysis models, we should do some more data exploration. First let us evaluate patterns of conspecific and heterospecific effects of soil microbes (boxplots of the conspecific effects of microbes ($m_{1A}$ and $m_{2B}$) vs heterospecific effects ($m_{1B}$ and $m_{2A}$)): 

```{r consp-v-heterosp-effect, eval = T}
live_ref %>%
...
There is one species pair with an extreme outlier in the heterospecific effects column. We need to look into what is happening here. I am fairly sure that this is the pair from McCarthy Neumann's study, where the paper has no value for plant growth, which was interpreted as zero growth in Crawford et al's dataset. 
...
Now that the data look in good shape, we are ready to quantify the effect sizes ((de)stabilization and fitness differences). To do so we use functions saved in the file `functions_effectSizes_coexOutcomes.R`. 

```{r calculate-effectsizes, eval = T, class.source = 'fold-show'}
# Source the functions
# source("analysis/functions_effectSizes_coexOutcomes.R")
# The function makes a new dataframe, so let's use that to override the
# existing one.
live_ref <- calculate_effect_sizes(live_ref)
# The new columns are: 
# mean_SD, IS, var_SD, sem_SD, n_min_SD; mean_FD, var_FD, sem_FD,
# n_min_FD; abs_FD, mean_IGR, var_IGR, sem_IGR
...
Right now the datasets are in "wide" format, with each row containing a species pair with it's stabilization and fitness difference measures, and we need to make this into a long format dataframe with each species pair having two rows (one for stabilization, one for fitness differences).
...
live_ref_long <-
  ...
  # rename(FD_mean = mean_FD, #absFD_mean = abs(FD_mean),
  #        FD_var = var_FD, SD_mean = mean_SD, SD_var = var_SD) %>%
...
For the first question, we need to run a multi-variate meta-analysis model. To do this we first need to define a new function, because otherwise we would have to keep repeating a bunch of code. 
...
With the function defined, we can now run the meta-analysis model on the live reference dataset. 
...
coef_test(q1.lr.0.5, vcov = "CR2") %>% # check if different from zero
  kable () %>%
...
# Uncomment the following lines to re-run the profile plots -- commenting
# now because it's quite a slow step. 
...
Now we can quantify the heterogeneity. 
...
q1.xs.coef <- coef_test(q1.xs.0.5, vcov = "CR2") # check if different from zero
q1.xs.0.5_coefs <- data.frame(conf_int(q1.xs.0.5, vcov =  "CR2"))
# conf_int(q1.xs.0.5, vcov = "CR2") # check if different from zero
q1.xs.wald <- Wald_test(q1.xs.0.5, constraints = constrain_equal(1:2), vcov = "CR2") # check if values are different from one another
...
(q1_plot <- 
  ...
  ggpubr::geom_bracket(xmin = 0.8, xmax = 1.2, y = 5.14, 
                       label = "***", tip.length = c(0.01, 0.01)) +
  ggpubr::geom_bracket(xmin = 1.8, xmax = 2.2, y = 2.3, 
                       label = "***", tip.length = c(0.01, 0.01)) +
...
### Check sensitivity (Influence analysis)


```{r Cooks-distance-influence-analysis, eval = F}
# We can run the Cook's distances to identify "influential" experiments
cd.exp.q1.lr.0.5 <- cooks.distance.rma.mv(q1.lr.0.5, 
                                          cluster = live_ref_long$Experiment, 
                                          parallel = "multicore")
21

# identify which study  is the influential one
which(cd.exp.q1.lr.0.5 > 0.45) # study 143
live_ref_long[live_ref_long$Experiment == 143,] # Kandlikar 2021
...
# re-do analysis without study 143
live_ref_trim <- live_ref_long %>% filter(Experiment != 143)
q1.lr.0.5.trim <- run_metamod(which_df = live_ref_trim, 
                              which_response_column = "absolute_FDSD_effects",
                              moderators = " ~ (set-1)",
                              impute_cov_r = 0.5)

cooks.distance.rma.mv(q1.lr.0.5.trim, 
                      cluster = live_ref_trim$Experiment, 
                      parallel = "multicore")
live_ref_long[live_ref_long$Experiment == 138,] # detect another influential exp

# delete this other influential exp.:
live_ref_trim <- live_ref_long %>% 
  filter(Experiment != 138 & Experiment != 143)
q1.lr.0.5.trim <- run_metamod(which_df = live_ref_trim, 
                         which_response_column = "absolute_FDSD_effects",
                         moderators = " ~ (set-1)",
                         impute_cov_r = 0.5)
...
coef_test(q1.lr.0.5.trim, vcov = "CR2") # check if different from zero
conf_int(q1.lr.0.5.trim, vcov = "CR2") # take look at the confidence interval
q1.lr.trim.wald <- Wald_test(q1.lr.0.5.trim, constraints = constrain_equal(1:2), vcov = "CR2")


# find "influential points"
cd.points.q1.lr.0.5 <- cooks.distance.rma.mv(q1.lr.0.5, parallel = "multicore")
which(cd.points.q1.lr.0.5>0.45)

plot(cd.points.q1.lr.0.5, type = "o", pch = 19, xlab = "Points", ylab = "Cook's Distance")
abline(0.45, 0, col = "red")



# Now for sterile soils -------

# influence test
# find "influential experiments"
cd.exp.q1.xs.0.5<-cooks.distance.rma.mv(q1.xs.0.5, 
                      cluster = sterile_ref_long$Experiment, #whereas influential studies = 51, 172
                      reestimate = F,
                      parallel = "multicore")

which(cd.exp.q1.xs.0.5>0.45) # influential experiment = 33
...
# remove influential experiment 33
sterile_ref_trim <- sterile_ref_long %>% filter(Experiment != 33)
q1.xs.0.5.trim <- run_metamod(which_df = sterile_ref_trim, 
                         which_response_column = "absolute_FDSD_effects",
                         moderators = " ~ (set-1)",
                         impute_cov_r = 0.5)
# check if there are new influence experiments:
cooks.distance.rma.mv(q1.xs.0.5.trim, 
                      cluster = sterile_ref_trim$Experiment, 
                      reestimate = F,
                      parallel = "multicore") # no other influence experiments

coef_test(q1.xs.0.5.trim, vcov = "CR2") # check if different from zero
conf_int(q1.xs.0.5.trim, vcov = "CR2") # take look at the confidence interval
q1.xs.trim.wald <- Wald_test(q1.xs.0.5.trim, constraints = constrain_equal(1:2), vcov = "CR2")
...
cd.points.q1.xs.0.5 <- cooks.distance.rma.mv(q1.xs.0.5, parallel = "multicore",
                                           reestimate = F)
which(cd.points.q1.xs.0.5 > 0.45) # no influential points

plot(cd.points.q1.xs.0.5, type = "o", pch = 19, xlab = "Points", ylab = "Cook's Distance")
abline(0.45, 0, col = "red")
...
#### Live reference influence analysis

In the influence analysis for live soil references we found two studies that were influential; we replicated all analyses without these studies (expand code chunk below for details).
...
# which(cd.exp.q1.lr.0.5 > 0.45) # exp.143
# live_ref_long[live_ref_long$Experiment == 143,] # Kandlikar et al
...
cd.q1.lr.0.5.no143 <- cooks.distance.rma.mv(q1.lr.0.5.no143, 
                                            ...
                                            parallel = "multicore")
# which(cd.q1.lr.0.5.no143 > 0.45) # detect another influential experiment: exp.138
# live_ref_long[live_ref_long$Experiment == 138,]
...
q1.lr.trim.wald <- Wald_test(q1.lr.0.5.no143.138, constraints = constrain_equal(1:2), vcov = "CR2")

# find "influential points"
# the code below was run and saved, here just being sourced to save time
# cd.points.q1.lr.0.5 <- cooks.distance.rma.mv(q1.lr.0.5, parallel = "multicore")
# write.csv(cd.points.q1.lr.0.5, "cd.points.q1.lr.0.5.csv")
cd.points.q1.lr.0.5 <- read.csv("cd.points.q1.lr.0.5.csv", row.names = 1)
# which(cd.points.q1.lr.0.5 > 0.45) # no "influential points"
```

#### Sterile reference influence analysis

In the influence analysis for live soil references we found one study that was influential; we replicated all analyses without these studies (expand code chunk below for details).
...
#                       reestimate = F,
#                       parallel = "multicore")
# write.csv(cd.exp.q1.xs.0.5, "cd.exp.q1.xs.0.5.csv")
cd.exp.q1.xs.0.5 <- read.csv("cd.exp.q1.xs.0.5.csv")
colnames(cd.exp.q1.xs.0.5) <- c("expt", "cooks_dist")

# cd.exp.q1.xs.0.5[cd.exp.q1.xs.0.5$cooks_dist>0.45, ] # influential experiment = 33; Hendricks et al.
#sterile_ref_long[sterile_ref_long$Experiment ==33,] %>% view()
...
q1.xs.trim.wald <- Wald_test(q1.xs.0.5.no33, constraints = constrain_equal(1:2), vcov = "CR2")


# find "influential points"
# cd.points.q1.xs.0.5<-cooks.distance.rma.mv(q1.xs.0.5, parallel = "multicore",
#                                            reestimate = F)
# write.csv(cd.points.q1.xs.0.5, "cd.points.q1.xs.0.5.csv")
cd.points.q1.xs.0.5 <- read.csv("cd.points.q1.xs.0.5.csv")
...
Now, explore how experimental moderators affect the outcomes
...
***Now, put it all together to make the plot. ***
...
# Check which comparisons are possible to make in this dataset:
# table(sterile_ref$`Training environment`) # 177 pairs in field, 235 in lab, 34 in LabField
...
# table(sterile_ref$control_type)
```
...
Across species pairs, what is the net effect of microbes on competitive outcomes? In other words, how frequently does the balance of microbially-mediated stabilization (or destabilization) and fitness differences predict species coexistence, exclusion, or priority effects


```{r make-base-coex-plot, eval = T}
...
### Start sampling approach 

To do this sampling we use the custom function `simulation_ci`, which we imported in sourcing the function file above. 
...
# Define a new function to summarize outcomes
...
FL_simulation_out_FILow <- simulation_ci(data_with_SDFD = live_ref %>% filter(`Fraction inoculum` < 0.251))
...
XS_simulation_out_FILow <- simulation_ci(data_with_SDFD = sterile_ref %>% filter(`Fraction inoculum` < 0.251))
...
XS_simulation_out_SterileOther <- simulation_ci(data_with_SDFD = sterile_ref %>% filter(!(Study_sterilization_method %in%  
                                                                                         c("autoclaving", "gamma irradiation", "heating"))))
...
#### **Table of outcomes for live soil reference**

```{r make-simulation-table-live, eval = T}
# library(kableExtra)
...
#### **Table of outcomes for sterile soil reference**
...
Here we do separate meta-analysis models for species pairs each each dataset with stabilizing or destabilizing effects of soil microbes

### Run meta-analysis for live ref soil
```{r Q2-meta-analysis-live,  eval = T, cache = T}
live_ref_long_q2 <- 
  live_ref_long %>% 
...
```{r Q2-meta-analysis-live-results, eval = T, results = 'asis', class.source = 'fold-show'}
# check if different from zero
coef_test(q2.lr.0.5, vcov = "CR2") %>% 
  kable() %>%
  kable_styling(full_width = 0)
# check if values are different from one another
...
#### Run meta-analysis for sterile ref soil
```{r Q2-meta-analysis-sterile,  cache = T, eval = T}
sterile_ref_long_q2 <- 
  sterile_ref_long %>% 
  # Get rid of the rows where SD < 0
  filter(set == "FD" | (set == "SD" & mean_value > 0)) %>%
...
```{r Q2-meta-analysis-sterile-results, eval = T, results = 'asis', class.source = 'fold-show'}
# check if different from zero
...
**For pairs with stabilizing effects (both reference types)**
...
### Univariate version of the Q1 models
...
### Univariate analysis of the strength of (de)stabilization
...
# Univariate analysis of the (De)stabilization metric
...
Now we can make an orchard plot of the (de)stabilization metric. 
...
### Evaluate sensitivity of Q1 models to the imputed value of within-experient correlation

Here we run models as in the main text, but we impute covariance matrices with assumed within-experiment (de)stabilization-fitness difference correlations of -0.5, 0.1, or 0.9. (The main text models are run with assumed correlations of 0.5.). Expand code below to see details. 
...
*First, assume negative values for all $m$ terms*

```{r conceptual-fig1, eval = T, cache = T, warning = FALSE, fig.height = 4, fig.width = 8}
...
out_coex <- data.frame(deSolve::ode(func = psf_model,
                                    y = init, times = time, parms = params_coex)) 
out_excl <- data.frame(deSolve::ode(func = psf_model,
                                    y = init, times = time, parms = params_excl)) 
out_priority <- data.frame(deSolve::ode(func = psf_model,
                                        y = init, times = time, parms = params_priority)) 

out_priority2 <- data.frame(deSolve::ode(func = psf_model,
                                         y = c(pA = 0.7, pAlpha = 0.7), times = time, parms = params_priority)) 
out_excl2 <- data.frame(deSolve::ode(func = psf_model,
                                     y = init, times = time, parms = params_excl2)) 
out_excl2b <- data.frame(deSolve::ode(func = psf_model,
                                      y = c(pA = 0.7, pAlpha = 0.7), times = time, parms = params_excl2)) 
...
(pp2 <- 
  ...
  annotate("text", x = -0.9, y = 0.35, hjust = 0, 
           label = "Priority\neffects", size = 5) +
    # geom_point(#"text", label = "outcome\n1", fontface = "italic", color = "midnightblue",
    #          aes(x = metrics(params_excl2)["SD"], 
...
Make the conceptual figure, but this time, show all four outcomes using positive values of all $m$ terms. 
```{r conceptual-fig1-mutualisms, eval = T, cache = T, warning = F, fig.height = 4, fig.width = 8}
...
pp2 <- 
    ...
    labs(
      x = "<span style = 'font-size:10pt;color:grey25;'>
    *Do plants grow better (destabilizing) or worse (stabilizing)<br>
    in soils conditioned by conspecifics vs. heterospecifics?*
    </span>",
...
### analysis to justify Q2 pairwise approaches
```{r q2-reason, eval = TRUE, cache = T}
set.seed(680)
...
### Catterpillar-like plots
```{r catterpillar-plots, eval = T, fig.height = 8}
...
```{r catterpillar-plots-2, eval = T, fig.height = 20}
...
Finally we want to just double check that our data extraction protocol to get the estimates of plant growth in reference soils gives the same results as Crawford et al's extractions (when we apply our protocol to non-reference growth).

```{r compare-data-extractions, eval = T, message=FALSE}
