...
# `R` code to recreate all results 
...
### Figure 1
*For Figure 1, we assume negative values for all $m$ terms*
...
# if directly run the code in console (instead of knitting RMD), use:
# live_ref <- read_csv("data/data_live_ref.csv")
# similar for reading other data 
...
Now that the data look in good shape, we are ready to quantify the effect sizes ((de)stabilization and fitness differences). To do so we use functions defined in the earlier section "Define functions & prepare data". 
...
# Warning: this chunk of code can take ~10 min to run
...
# Warning: this chunk of code can take ~5 min to run
# Control type ---------
...
# Warning: this chunk of code can take ~15 min to run
...
#### Influence test for model with live reference data
...
# write.csv(cd.points.q1.lr.0.5, "../data/influence_test_results/cd.points.q1.lr.0.5.csv")
cd.points.q1.lr.0.5 <- read.csv("../data/influence_test_results/cd.points.q1.lr.0.5.csv", row.names = 1)
# which(cd.points.q1.lr.0.5 > 0.45) # no "influential points"
```

#### Influence test for model with sterile reference data

In the influence analysis for sterile soil references we found one study that was influential; we replicated all analyses without these studies (expand code chunk below for details).
...
#                       reestimate = F,
#                       parallel = "multicore")
# write.csv(cd.exp.q1.xs.0.5, "../data/influence_test_results/cd.exp.q1.xs.0.5.csv")
# otherwise the above chunk can take hours to run
cd.exp.q1.xs.0.5 <- read.csv("../data/influence_test_results/cd.exp.q1.xs.0.5.csv")
...
#                                            reestimate = F)
# write.csv(cd.points.q1.xs.0.5, "../data/influence_test_results/cd.points.q1.xs.0.5.csv")
# directly source outcomes to avoid excessive run time (can take hours)
cd.points.q1.xs.0.5 <- read.csv("../data/influence_test_results/cd.points.q1.xs.0.5.csv")
...
#### Make plots with & without influential points/expts (Fig.S.6)
...
### Fig.S.7: Meta-analysis approach
...
### Fig.4: Sampling approach 

To do this sampling, we use the custom function `simulation_ci`, which we have defined in the earlier section "Define functions & prepare data". 
...
### Fig.S.8: analysis to justify Q2 pairwise approaches
