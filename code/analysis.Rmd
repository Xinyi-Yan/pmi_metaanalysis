...
The first goal is to make a base plot of stabilization/fitness differences onto which we can add species pairs. We will use this base plot to make the middle panel of Figure 1, and also to make Figure 4 in the paper. 
...
Figure 1 focuses on dynamics when microbes tend to hurt plant growth (negative values for all $m$ terms). Figure S1 illustrates how all outcomes are possible even when microbes benefit plant growth (positive values for all $m$ terms).
...
# run psf_model() using deSolve::ode()
...
```{r conceptual-fig1-mutualisms, eval = T, cache = T, warning = FALSE, fig.height = 4, fig.width = 8}
...
### Pairwise direct comparison (of ND and FD)

We first evaluate the pairwise competitive outcome by directly comparing the mean niche and fitness differences within species pairs.
...
### Fig.4: Pairwise sampling approach 
