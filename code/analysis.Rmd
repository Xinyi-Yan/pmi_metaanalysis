...
The html document mostly shows the output rather than the underlying code. Click on the 'Code' button to the right to expand or hide the code.
...
We make another conceptual figure, but this time, show all four outcomes using positive values of all $m$ terms. 
...
Now that the data look in good shape, we are ready to quantify the effect sizes (magnitude of (de)stabilization and fitness differences). To do so we use functions defined in the earlier section "Define functions & prepare data". 
...
Now we can quantify the heterogeneity of the effect sizes. 
...
Now, we make a figure that visualize the effect sizes (magnitude of SD, FD), their CIs, and their heterogeneities for both datasets.
...
Now, explore how experimental moderators affect the outcomes using mixed effect models.
...
To test robustness of our results, we alternatively run univariate models of just SD or FD, and compare their magnitudes.
...
First, before looking into each individual species pairs, we start with a meta-analytical approach like Q1 to predict the overall competitive outcome given the *average* microbial effect. Here we do separate meta-analysis models for species pairs each each dataset with stabilizing or destabilizing effects of soil microbes
...
Wald_test(q2.xs.0.5, constraints = constrain_equal(1:2),  vcov = "CR2") %>% 
  ...
  kable_styling(full_width = F)
```
for pairs where interactions are stabilized, their overall FD exceed the magnitude of stabilizing effect
...
To evaluate the pairwise competitive outcome, We first do so by directly comparing the mean niche and fitness differences within species pairs.
...
We further incorporate empirical uncertainties into the inference via a sampling approach. To do this sampling, we use the custom function `simulation_ci`, which we have defined in the earlier section "Define functions & prepare data". 
...
Looking at competitive outcomes at the species pair level can reveal patterns that may otherwise hidden using the meta-analytical approach. For example, even when microbes drive stronger fitness differences than stabilization on average, they can still favor coexistence for a substantial fraction of species pairs, especially if these effects are negatively correlated. Here we illustrate this scenario.  
